...
- **顺手单**（🧪 D3b Done 复核看 revert 率；判据与机械传棒同源）：**机械修正**——由
  已有裁决/规则/事实唯一确定、无实现方式选择——可由对方顺手代做，免一次往返。
  三条件缺一不可：①机械性 ②单独 commit 标 `[顺手]` + review/devlog 记一行索引，
  下轮对方必过目 ③不碰对方未收口增量涉及的文件。**廉价否决**：域 owner 可无理由
