...
"如果没有 owner，我本来会怎么判"。发起方先在工作仓 review / plan 写完整记录；owner 轨
收到 notification 后同步一行到 `calibration/decision-log.md`：
...
- Delivery Owner escalate → 门铃 Spec Owner；Spec Owner 只做三个**加法**动作：复核事实、附议或
  ≤10 行反对立场、备齐中央账索引字段 / 按需建卡 → 整理包 notification 给 owner；
...
**两层单写**（✅）：工作仓完整记录由该 batch 的 Spec Owner 统一写；duet 中央
decision-log 由 owner 轨收到 notification 后同步，一次 escalation 一行。发起方是 Delivery
Owner 时建议列原样摘录其 verdict。batch 不跨仓写 duet；两处长文必漂移，且 duet 是私有仓，
写进来的内容团队永远看不见。
...
| owner 收件箱 | owner 轨收到 escalate 后同步的 decision-log ⬜ 行（天然聚合） |
