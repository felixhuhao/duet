# 🧪 试行条款集中结算（2026-08-16，并轨结算点）

防杂宪章条款 ②：试行条款在复核点结算——**fire 过的转正（去 🧪 标），没 fire 过的撤销**。
下表为结算底稿；「转正」由 owner 批准后逐文件去标。

| # | 条款（所在文件） | fire 记录 | 建议 |
|---|---|---|---|
| 1 | verdict 前置两动作（verdict.md） | 立据事故 ×2（D3b PASS 漏 §8、E-7 漏引用）；AGT-1 起按其执行，authority 复核抓到 205-commit 落差 | **转正** |
| 2 | 结论范围 ≤ 证据范围（roles×2） | 立据日四次同形；后又抓 B pane keychain 误判、D pane「grep 当全集」（`b357a2cd`） | **转正** |
| 3 | 交棒是 push / 禁监听（baton.md） | 全部传棒按此执行，无回退 | **转正** |
| 4 | 传棒分级·机械段（baton.md） | 大量执行，零传错、零降级 | **转正** |
| 5 | 收棒队列（baton.md） | Codex Tab 实测 QUEUED-OK；Claude 边界注入实测（w6）；日常使用 | **转正** |
| 6 | 送达验证（baton.md） | 首测即抓真故障（sandbox 拦 herdr）；此后按「零退出+读回」执行 | **转正** |
...
结算动作：owner 批准后，1–12 全部去 🧪 标（文字不改）。
