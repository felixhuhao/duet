...
| default session空 workspace | raw CLI mutation漏写 `--session mobile-solo-pilot` | 在 default创建一个空 `wD`，随即关闭；无 agent、无仓库写入 | owner轨 mutation一律 `herdr --session <name> ...`；跨session优先 wrapper |
...
- 本轮直接出现：1个错误session空workspace、1次name_taken、1次delivery readback失败；均未造成仓库数据
  丢失、重复agent或重复任务消费。
